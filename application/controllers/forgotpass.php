...
/**
 * Created by PhpStorm.
 * User: Imported Library By Danish Qaim Khani  
 * Date: 1/29/2019
 * Description : This controller is being used for Manage Forget Password 
 */
    ...
    public function index()
    {
        ...
        if ($_POST) {
            ...
            if (($user && $usertrue == '1' && $user->status == 'active') || ($user && $usertrue == '0' && $user->inactive == '0')) {
                $token = hash('sha256', md5($timestamp . $user->id . $user->personname));
                $contact_name = $user->personname ;
                ...
                $this->email->from($data['core_settings']->email, $data['core_settings']->company,0);
                if($parentEmail=="")
        ...
        // override image link because image was not opening  in email because of server issue 
            ...
            if($parentEmail==""){
                ...
                $email = read_file('./application/views/' . $data['core_settings']->template . '/templates/email_pw_reset_link_initiator.html');
            }
